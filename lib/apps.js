...
Apps.prototype.remove = function(name, cb) {
  ...
  self.stop(name, function(err, app) {
    ...
    if (!app) return cb && cb(new Error('Unable to find app'))
    var plistPath = app.plistPath
    app.remove(function(err) {
      if (err) return cb && cb(err)
      fs.unlink(plistPath, cb)
    })
  })
}
...
Apps.prototype.generatePlist = function(app, cb) {
  ...
  var opts = {};
  var fullPidPath = path.join(self.mapps.config.root, app.pidFile)
  opts[fullPidPath] = true
